...
%RB: no testing if there are at least 2 classes
% 
% 	W = NAIVEBC(A,N)
% 	W = A*NAIVEBC([],N)
%
...
function w = naivebc_rb(a,arg2)
	...
	if (~ismapping(arg2))     % Second argument is not a mapping: training.
		...
		if isnan(N)             % optimize complexity parameter
			...
			offset_a = min([a;a]); maxa = max([a;a]); scale_a = maxa - offset_a; %RB
            if unique(scale_a)==0  %RB
                offset_a=zeros(1,k);  %RB
                scale_a=min([a;a]);  %RB
            end  %RB
...
% 			end
			...
			pars.p0 = getprior(a,0); pars.p = p; pars.N = N;
