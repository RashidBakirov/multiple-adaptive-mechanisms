...
class LeverageBagging(BaseSKMObject, ClassifierMixin, MetaEstimatorMixin):
    ...
    #not doing anything (0 and 3 are reversed?)
    #LB_MODE = 0
    #updating the models
    #LB_MODE = 1
    #only adding a new model
    #LB_MODE = 2
    #updating models AND adding a new model
    #LB_MODE = 3
    #updating models AND adding a new model with change detection (original LB)
    #LB_MODE = 4
    ...
    def partial_fit(self, X, y, LB_MODE, classes=None, sample_weight=None):
    ...
    def __partial_fit(self, X, y):
        ...
        for i in range(self.actual_n_estimators):
            for j in range(r):
                if self.LB_MODE != 2:
                        k = self.random_weight()

                        if k > 0:
                            if self.enable_code_matrix:
                                y_cp = self.matrix_codes[i][int(y_cp)]
                            for l in range(int(k)):
                                self.ensemble[i].partial_fit(np.asarray([X_cp[j]]), np.asarray([y_cp[j]]), self.classes)

                if self.LB_MODE == 4 or self.LB_MODE == 2 or self.LB_MODE == 0:
                    try:
                        pred = self.ensemble[i].predict(np.asarray([X_cp[j]]))
                        if pred is not None:
                            add = 0 if (pred[0] == y_cp[j]) else 1
        ...
        if (self.LB_MODE == 4 and change_detected) or self.LB_MODE == 2 or self.LB_MODE == 0:
            ...
            if i_max != -1:
                ...
                if self.enable_code_matrix:
                    y_cp = self.matrix_codes[i_max][int(y_cp)]
                for j in range(r):
                    k = self.random_weight()
                    if k > 0:
                        for l in range(int(k)):
                            self.ensemble[i_max].partial_fit(np.asarray([X_cp[j]]), np.asarray([y_cp[j]]), self.classes)
        return self
