function [ fin_preds, new_ensemble ] = wm_mult_predict( ensemble, data, true_labels )
...
%ensemble of experts with weights. For multiclass prediction
    ...
    for j=1:ens_size
        ...
        predvector(j)=pred; %get predicted label   
    ...
    labels_cube = repmat(labels, [size(predvector, 2), 1]);new_ensemble{1}.ensemble_accuracy=mean(true_labels==fin_preds);
