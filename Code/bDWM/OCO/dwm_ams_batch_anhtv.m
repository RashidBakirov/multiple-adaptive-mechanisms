function[acc, avg_acc , result, preds, avg_acc_test, pred_test, ams] = dwm_ams_batch_anhtv(data, labels, classifier, batchsize, test_data, test_labels)
tic
% dynamic weighted majority with multiple ams
% ANH.TV implementation
...
for k = 2:floor(rows/batchsize)
    ...
    regret(k-1,:)=repmat(total_weighted_loss,1,size(loss,2))-loss; %regret for each expert
    c(k-1,:)=abs(regret(k-1,:));
