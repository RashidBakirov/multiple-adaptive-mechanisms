function[acc, avg_acc , result, preds, avg_acc_test, pred_test, ams, times] = dwm_ams_batch_lite_xvpar(data, labels, classifier, mode, flag_rc,  batchsize, test_data, test_labels, folds)

% p - parallell implementation using parfors
...
ds2=dataset(data(batchsize+1:batchsize+batchsize,:),labels(batchsize+1:batchsize+batchsize)) %create the first datapoint
dsk=[];
cl1=ds1*classifier;
cl2=ds1*classifier;
if flag_rc
    for i=1:2
        ...
        ensemble{i}{1}.model=cl1;
        ensemble{i}{2}.model=cl2;
        ensemble{i}{1}.weight=1;
        ensemble{i}{2}.weight=1;
        ensemble{i}{1}.data=ds1;
        ensemble{i}{2}.data=ds2;
    end
else
    selected_ensemble{1}.model=cl1;
    selected_ensemble{2}.model=cl2;
    selected_ensemble{1}.weight=1;
    selected_ensemble{2}.weight=1;
    selected_ensemble{1}.data=ds1;
    selected_ensemble{2}.data=ds2;
end
...
for k = 3:floor(rows/batchsize)
    ...
    if flag_rc
        ...
        %RC
        r_actual=r;
        [selected_ensemble, r] = dwm_candidate_select_batch_lite_p(ensemble);
        %[selected_ensemble, r] = dwm_candidate_select_batch_lite(ensemble);
    ...
    if mode == 10
        if flag_rc
            size1 = size(ensemble{r})
            [xval_selection] = dwm_xval_select_batch_lite_p(ensemble{r}, dataBatch, labelsBatch, classifier, batchsize, NUMFOLDS);
        else
            size1 = size(selected_ensemble)
            [xval_selection] = dwm_xval_select_batch_lite_p(selected_ensemble, dataBatch, labelsBatch, classifier, batchsize, NUMFOLDS);
        end
    end
    ...
    if flag_rc
        ...
        for i=1:2
            ensemble{i}=dwm_adapt_batch_lite(selected_ensemble,dsk,classifier,i);
        end
    ...
    elseif mode~=11 %if not set selected_ensemble auomatically
      ...
      elseif mode==9 %original dwm
        ...
        selected_ensemble=dwm_adapt_batch_lite(selected_ensemble,dsk,classifier,r);       
    end
