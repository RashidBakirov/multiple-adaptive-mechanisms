...
for k = 2:floor(rows/batchsize)
    ...
    if flag_rc
        ...
        if mode<=7 %always same AM
            r=mode+1;
            disp([num2str(k) ': AM deployed = ' num2str(r-1)]);
        elseif mode==8 %random AM
            r=randi(8);
            disp([num2str(k) ': AM deployed = ' num2str(r-1)]);
    ...
    if flag_rc
        for i=0:7
            ensemble{i+1}=dwm_adapt_batch(selected_ensemble,dsk,classifier,i);
        end
