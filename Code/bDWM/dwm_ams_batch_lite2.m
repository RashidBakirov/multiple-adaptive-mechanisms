function[acc, avg_acc , result, preds, avg_acc_test, pred_test, ams, times] = dwm_ams_batch_lite2(data, labels, classifier, mode, flag_rc,  batchsize, test_data, test_labels, folds)
...
for k = 3:floor(rows/batchsize)
    ...
    if mode == 10
        if flag_rc
            [xval_selection] = dwm_xval_select_batch_lite(ensemble{r}, dataBatch, labelsBatch, classifier, batchsize, NUMFOLDS);
        else
            [xval_selection] = dwm_xval_select_batch_lite(selected_ensemble, dataBatch, labelsBatch, classifier, batchsize, NUMFOLDS);
        end
    end
