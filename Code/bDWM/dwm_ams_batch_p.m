function[acc, avg_acc , result, preds, avg_acc_test, pred_test, ams, time] = dwm_ams_batch_p(data, labels, classifier, mode, flag_rc,  batchsize, test_data, test_labels, folds)
tic
...
for k = 2:floor(rows/batchsize)
    ...
    if mode == 10
        if flag_rc
            [xval_selection] = dwm_xval_select_batch_par2(ensemble{r}, dataBatch, labelsBatch, classifier, batchsize, NUMFOLDS);
        else
            [xval_selection] = dwm_xval_select_batch_par2(selected_ensemble, dataBatch, labelsBatch, classifier, batchsize, NUMFOLDS);
        end
    end
