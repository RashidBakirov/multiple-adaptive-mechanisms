...
% Written by Matthew Dunham
    ...
    %addition by rb, if feature names are empty, name them 1,2,3,...
    if (isempty(featureNames))
        featureNames = 1:size(data,2);
        featureNames = sprintfc('%d',featureNames);
    end
    ...
    vec = javaObject('weka.core.FastVector');
    if(iscell(data))
        for i=1:numel(featureNames)
            if(ischar(data{1,i}))
                attvals = unique(data(:,i));
                values = javaObject('weka.core.FastVector');
                for j=1:numel(attvals)
                   values.addElement(attvals{j});
                end
                vec.addElement(javaObject('weka.core.Attribute', featureNames{i}, values));
            else
                vec.addElement(javaObject('weka.core.Attribute', featureNames{i})); 
            end
        end 
    else
        for i=1:numel(featureNames)
    ...
    wekaOBJ = javaObject('weka.core.Instances', name, vec, size(data,1));
    if(iscell(data))
        for i=1:size(data,1)
            inst = javaObject('weka.core.DenseInstance', numel(featureNames));
        ...
        for i=1:size(data,1)
            wekaOBJ.add(javaObject('weka.core.DenseInstance', 1, data(i,:)));
        end
    end
