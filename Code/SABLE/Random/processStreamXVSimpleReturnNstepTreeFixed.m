function [ avg_error, predictions, errors, mseVal, mapeVal, maseVal, deployedAMSeq, retainedAMSeq ] = processStreamXVSimpleReturnNstepTreeFixed( experts, mutualPDF, mutualPDFPos, data, val, batchSize, nSteps, AMs)
...
%             retainedAMSeq=[retainedAMSeq;seq];
        ...
        for i = iterator
            ...
            if modelsTree.isleaf(i)==1
                ...
                adaptedModel=model;
                adaptedModel.AM=0;
                [modelsTree] = modelsTree.addnode(i, adaptedModel);
                if (isequaln(seq,retainedAMSeq(end,:)) || isequaln(retainedAMSeq, nan(1, nSteps))) && isequal(AMs,0)                 
                ...
                if (isequaln(seq,retainedAMSeq(end,:)) || isequaln(retainedAMSeq, nan(1, nSteps))) && isequal(AMs,1)  
                    currentModel=adaptedModel;
                end
                ...
                if (isequaln(seq,retainedAMSeq(end,:)) || isequaln(retainedAMSeq, nan(1, nSteps))) && ismember(2,AMs)  
                    currentModel=adaptedModel;
                    if isequal(AMs,[2 4])  
                        [ currentModel.experts, currentModel.mutualPDF, currentModel.mutualPDFPos ] = generateExpertsAndPruneSimple( currentModel.experts, currentModel.mutualPDF, currentModel.mutualPDFPos, ...
                        ...
                        [currentModel.mutualPDF, currentModel.mutualPDFPos ]  = updateDescriptorsBatchB( currentModel.experts, dataBatch, valBatch,currentModel.mutualPDF, currentModel.mutualPDFPos,  par.adaptDescriptorKernelSize );
                ...
                if (isequaln(seq,retainedAMSeq(end,:)) || isequaln(retainedAMSeq, nan(1, nSteps))) && isequal(AMs,3)  
                    currentModel=adaptedModel;
                end
                ...
                adaptedModel.AM=4;
                [modelsTree, idx] = modelsTree.addnode(i, adaptedModel);
                if (isequaln(seq,retainedAMSeq(end,:)) || isequaln(retainedAMSeq, nan(1, nSteps))) && isequal(AMs,4)  
                    currentModel=adaptedModel;
                end
