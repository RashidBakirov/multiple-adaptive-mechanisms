    ...
    for t=1:3
        for q=0:1           
            data = res_all{t}(1:26,1+q*4:4+q*4);
            ...
            ylim([0,27])
            %pbaspect([1 2 1])
            title(['bPL synth data. Threshold: ' num2str(2*(t-1)) ', Batch size:' num2str(10*(1+q))]);
            saveas(fig,['results/bPL_synth_' num2str(2*(t-1)) '_' num2str(10*(1+q)) '.jpg']);
        end
    end
