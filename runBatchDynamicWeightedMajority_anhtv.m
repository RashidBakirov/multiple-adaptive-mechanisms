...
function runBatchDynamicWeightedMajority_anhtv(datasetIdentifier, batchSize)
...
for i=1:100
    disp(['Run: ' num2str(i)]);
    [acc{i}, avg_acc(i) , ~, ~, avg_acc_test(i), ~, ams{i}] = dwm_ams_batch_anhtv(data, labels, naivebc_rb, batchSize, test_data, test_labels);
    save (['results/oco/DWMAMS_anhtv_' num2str(datasetIdentifier) '_' num2str(batchSize) '.mat'], 'acc', 'avg_acc', 'avg_acc_test', 'ams');
end
